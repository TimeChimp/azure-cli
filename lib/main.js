...
const run = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        */
        ...
        command += `-e GITHUB_WORKSPACE=${CONTAINER_WORKSPACE} -e GITHUB_SHA=${process.env.GITHUB_SHA} -e $GITHUB_REF=${process.env.GITHUB_REF} `;
        command += `--name ${CONTAINER_NAME} `;
        command += ` mcr.microsoft.com/azure-cli:${azcliversion} ${startCommand}`;
        console.log(`${START_SCRIPT_EXECUTION_MARKER}${azcliversion} somehthing: ${process.env.GITHUB_REF}`);
